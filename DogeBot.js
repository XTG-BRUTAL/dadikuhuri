...
//or copy the codes,
//pls give credit
//© 2022 BRUTAL Bot Inc. DADI KUHURI
...
developerNo = '919101765679'
developerName= '🦄Dream Guy brutal'
...
╰───「 *🥳ʟᴇᴠᴇʟ ᴜᴘ🥳* 」`)
          ...
          sendButton(from, lvlup, '*DADI KUHURI*', but)
...
Please Choose Whether Document, Audio or Video Below`
        sendButMessage(from, result, `*DADI KUHURI*`, [
          {
...
sendButVideo(from, pll, `*DADI KUHURI*`, get_result, [      
{
buttonId: `${prefix+command}`,
buttonText: {
displayText: `NEXT ➡️`,
...
case 'memegenerator': case 'memegen':{
 	...
 	case 'stickermeme': case 'memesticker': case 'memestick': case 'stickmeme': case 'stcmeme': case 'smeme':{
						if (args.length < 1) return reply(`Send orders *${prefix + command}* DADI KUHURI`)
									if (q.includes('|')) return reply(`Reply to an image with a caption, For Example *${prefix + command}* BRUTAL`)
									try {
										if (!isQuotedImage) return reply(`Reply to an image!`)
										reply(mess.wait)
										var teks2 = args.join(' ')
...
//══════════[ GAME FEATURES ]══════════//
    ...
    "say i love the bot owner BRUTAL through voice note",
    ...
    "do you love the bot creator, BRUTAL?ðŸ¦„",
...
for (let _ of groupMembers) {
sendMess(_.jid, `*「 DADI KUHURI BROADCAST 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
}
